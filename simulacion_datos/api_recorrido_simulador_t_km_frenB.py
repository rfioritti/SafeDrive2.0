...
# si al dividir las distracciones entre el timpo del recorrido es menor al valor_distraccion entonces el dato va a ser un boolean 1 
...
for i in range(100):
    ...
    indice_distraccion = (tiempo_recorrido*0.19) + (km_recorridos*0.11) + (frenadas_bruscas*15)

    if indice_distraccion > 100 and random.random() <= 0.90:
        distracciones_del_Recorrido = tiempo_recorrido*valor_distraccion
    elif 75 <= indice_distraccion <= 100 and random.random() <= 0.60:
        distracciones_del_Recorrido = tiempo_recorrido*valor_distraccion
    elif 40 <= indice_distraccion < 75 and random.random() <= 0.30:
        distracciones_del_Recorrido = tiempo_recorrido*valor_distraccion
    elif indice_distraccion < 40 and random.random() <= 0.10:
        distracciones_del_Recorrido = tiempo_recorrido*valor_distraccion

    indice_dormido = (tiempo_recorrido*0.38) + (km_recorridos*0.23) + (frenadas_bruscas*22.50)
